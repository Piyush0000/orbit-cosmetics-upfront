...
export default function Header() {
    ...
    return (
        <>
                ...
                <div className="container mx-auto px-6 lg:px-10">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex flex-col">
                            <Link href="/" className="text-xl lg:text-2xl font-serif tracking-[0.2em] uppercase text-text-main leading-none">
                                {storeName}
                            </Link>
                            <span className="mono-label text-[8px] mt-1 text-text-muted">
                                Botanical Essence
                            </span>
                        </div>
                            ...
                            {NAV_LINKS.map((link) => (
                                <Link
                                    ...
                                    className={cn(
                                        ...
                                        // @ts-ignore
                                        link.highlight
                                            ? "px-6 py-2 bg-dream-violet text-black hover:bg-white hover:text-black rounded-full"
                        ...
                        <div className="flex items-center gap-5 transition-colors text-text-main">
                            <button className="p-2 hover:text-dream-violet transition-colors text-text-main" aria-label="Search">
                                <Search className="w-5 h-5 stroke-[1.5]" />
                            </button>
                            <Link href="/cart" className="p-2 hover:text-dream-violet transition-colors text-text-main" aria-label="Cart">
                            ...
                            {/* Mobile Menu Button */}
                            <button
                                className="lg:hidden p-2 hover:text-dream-violet"
            ...
            <div
                className={cn(
                    "fixed left-0 right-0 z-40 bg-dream-violet/10 backdrop-blur-md text-text-main text-center py-2 transition-all duration-500 border-y border-white/5",
            ...
            {/* Mobile Menu Overlay */}
            {mobileMenuOpen && (
                <div className="fixed inset-0 z-40 bg-background pt-32 px-8 lg:hidden animate-fade-in">
