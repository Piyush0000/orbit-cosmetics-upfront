...
export default function CheckoutSummary({ onPlaceOrder }: CheckoutSummaryProps) {
    ...
    return (
        <div className="glass-card p-10 rounded-[40px_10px_40px_10px] h-fit sticky top-32 border-white/5 relative overflow-hidden group shadow-2xl backdrop-blur-3xl">
            ...
            <h2 className="font-serif text-3xl text-text-main mb-10 pb-6 border-b border-white/5 italic tracking-tight relative z-10 leading-none">Order <span className="not-italic opacity-30">Selection</span></h2>
            ...
            <div className="space-y-6 mb-10 max-h-80 overflow-y-auto pr-4 custom-scrollbar relative z-10">
                {cartItems.map((item) => (
                    <div key={item.id} className="flex gap-4 group/item">
                        <div className="relative w-16 h-20 flex-shrink-0 glass-panel border-white/5 p-2 rounded-[15px_5px_15px_5px] transition-transform duration-500 group-hover/item:scale-105">
                            <Image
                                src={item.image}
                                alt={item.name}
                                fill
                        ...
                        <div className="flex-1 py-1">
                            ...
                            <p className="text-[8px] text-white/30 uppercase tracking-[0.2em] font-medium mt-1">{item.size}</p>
                            <p className="text-sm font-serif italic text-text-main mt-1.5 opacity-80">₹{(item.price * item.quantity).toLocaleString()}</p>
                        </div>
                    </div>
                ))}
            </div>
            ...
            <div className="mt-8 flex items-center justify-center gap-3 text-[8px] uppercase tracking-[0.3em] font-bold text-white/20">
