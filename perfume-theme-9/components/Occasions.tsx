            ...
            <div className="container mx-auto px-6 max-w-7xl relative z-10">
                <div className="text-center mb-24">
                    <div className="px-5 py-2 glass-card mb-8 inline-block shadow-sm">
                        <span className="text-[10px] tracking-[0.4em] uppercase block font-bold text-dream-violet">Perfect Moments • Occasion</span>
                    </div>
                    <h2 className="text-6xl md:text-[100px] font-serif text-text-main leading-none tracking-tighter italic">Signature <span className="not-italic opacity-40">Curation</span></h2>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
                    {OCCASIONS.map((occ, idx) => (
                        <Link
                            key={idx}
                            href={occ.href}
                            className="group relative h-[450px] overflow-hidden rounded-[30px_5px_30px_5px] cursor-pointer shadow-2xl transition-all duration-700 block border border-white/10"
                        >
                            {/* Background Image */}
                            <div
