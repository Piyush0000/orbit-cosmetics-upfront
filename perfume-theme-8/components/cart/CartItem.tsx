...
export default function CartItem({ item, onUpdateQuantity, onRemove }: CartItemProps) {
    return (
        <div className="flex gap-4 py-6 border-b border-gray-100 last:border-0 hover:bg-gray-50/50 transition-colors p-2 rounded-sm">
            {/* Image */}
            <div className="relative w-20 h-24 sm:w-24 sm:h-32 flex-shrink-0 bg-pastel-cream/30">
                {item.image ? (
                    <Image
                        src={item.image}
                        alt={item.name}
                    ...
                    <div className="flex items-center justify-center h-full text-pastel-rose">
