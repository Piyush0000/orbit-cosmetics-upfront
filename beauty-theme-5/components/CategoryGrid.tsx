...
const categories = [
    { name: "Skincare", icon: Sparkles, color: "bg-violet-50 dark:bg-violet-900/20", iconColor: "text-violet-500" },
    { name: "Haircare", icon: Scissors, color: "bg-rose-50 dark:bg-rose-900/20", iconColor: "text-rose-500" },
    { name: "Makeup", icon: Palette, color: "bg-fuchsia-50 dark:bg-fuchsia-900/20", iconColor: "text-fuchsia-500" },
    { name: "Hygiene", icon: Heart, color: "bg-pink-50 dark:bg-pink-900/20", iconColor: "text-pink-500" },
    { name: "Fragrances", icon: Flower2, color: "bg-purple-50 dark:bg-purple-900/20", iconColor: "text-purple-500" },
];
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div ref={scrollRef} className="flex gap-5 overflow-x-auto pb-4 scrollbar-hide snap-x snap-mandatory" style={{ scrollbarWidth: "none", msOverflowStyle: "none" }}>
                    {categories.map((cat) => (
                        <Card
                            key={cat.name}
                            className="group cursor-pointer hover:shadow-xl hover:shadow-primary/8 transition-all duration-500 border border-border/40 bg-card hover:-translate-y-1 rounded-3xl flex-shrink-0 w-[200px] snap-start"
                        >
                            <CardContent className="flex flex-col items-center justify-center p-8 gap-4">
                                <div className={`p-5 rounded-full ${cat.color} group-hover:scale-110 group-hover:rotate-6 transition-all duration-400`}>
